module Widgets
  module TabnavHelper  
    ...
    def tabnav name, opts={}, &block
      ...
      if block_given?
        #this just puts a main_tabnav_content dev around the content block
    ...
    def render_tabnav(name, opts={}, &proc)
      ...
      @_tabnav = Tabnav.new(name, opts) #makes a new
    ...
    def render_tabnav_tabs
      out tag('ul',  {:class => "ui-widget-header ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-corner-all"} , true)

      @_tabnav.tabs.each do |tab|      
        li_options = { :class=>"ui-state-default ui-corner-top  "}
        ...
        if tab.disabled?
          ...
          li_options[:class] = "ui-state-active ui-state-default ui-corner-top "
        end
        ...
        if tab.disabled? || (tab.link.empty? && tab.remote_link.nil?)
          ...
          out tab.button if !tab.button.nil?
